...
(async () => {
  ...
  // Click on "Calculadora Solar" or "Calcular Sistema" button
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
    const calcBtn = btns.find(b => b.textContent.includes('Comenzar Calculadora'));
  ...
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
    const add = btns.find(b => b.textContent.includes('Agregar Televisor'));
    if (add) add.click();
  });
  await new Promise(r => setTimeout(r, 500));
  await page.evaluate(() => {
  ...
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
    const next = btns.find(b => b.textContent.includes('Ver Resultados'));
    if (next) next.click();
  });
  await new Promise(r => setTimeout(r, 1500));

  // Screenshot before click
  await page.screenshot({ path: path.join(__dirname, 'before_click.png'), fullPage: true });
  ...
  await page.screenshot({ path: path.join(__dirname, 'after_click.png'), fullPage: true });
