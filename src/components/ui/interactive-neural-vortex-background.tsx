...
const InteractiveNeuralVortex = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Convoltaje Brand Colors: Primary Blue (0F3A7D) and Cyan (00D9FF)
        color = vec3(0.06, 0.23, 0.49); // Base dark blue
        color = mix(color, vec3(0.0, 0.85, 1.0), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2)); // Cyan mix
        color += vec3(0.0, 0.5, 0.8) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
    ...
    const render = () => {
      ...
      if (uPointerPosition) gl.uniform2f(uPointerPosition, 
  ...
  return (
    <canvas 
      ref={canvasRef} 
      id="neuro" 
      className="fixed inset-0 w-full h-full pointer-events-none opacity-40 z-[-1] bg-transparent"
    ></canvas>
  );
};
