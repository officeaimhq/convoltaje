...
(async () => {
  ...
  const refId = "OT-1407";
  const clientName = "Alina Quijano Marin";
  const clientPhone = "52831209";
  const clientEmail = "alina@example.com";
  const installationAddress = "Calle 19 #8819 entre 4ta y 6ta, Habana del Este";
  const installationDate = "sábado 4 de julio";
  const salesAgent = "Juan Comercial";
  
  const kit = {
    name: "Sistema Solar Medio 3000W",
    description: "Ideal para casas con consumo moderado.",
    inverterPower: 3,
    batteryCapacity: 5,
    price: 3850
  };
  const dailyConsumption = 12.5;
  ...
  await page.pdf({ path: 'prefactura_OT-1407_Alina_Quijano.pdf', format: 'A4' });
  await browser.close();
})();
